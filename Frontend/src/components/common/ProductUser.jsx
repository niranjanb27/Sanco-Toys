          ...
          <div className="flex md:flex-col gap-2 overflow-x-auto">
            {images.map((img, idx) => (
              <img
                ...
                className={`w-14 h-14 object-cover rounded-lg cursor-pointer border transition ${
                  selectedMedia === img ? "border-blue-500" : "border-gray-300"
                } hover:opacity-80`}
              />
            ))}
            {videos.length > 0 &&
              videos.map((vid, idx) => (
                <video
                  key={idx}
                  className={`w-14 h-14 object-cover rounded-lg cursor-pointer border transition ${
                    selectedMedia === vid ? "border-blue-500" : "border-gray-300"
                  } hover:opacity-80`}
        ...
        <div className="w-full md:w-1/2 space-y-5">
          ...
          <div className="flex items-center gap-3">
            ...
            <input
              ...
              className="w-16 border rounded-md p-1 text-center focus:outline-none focus:ring-2 focus:ring-blue-400"
              min="1"
            />
          </div>
